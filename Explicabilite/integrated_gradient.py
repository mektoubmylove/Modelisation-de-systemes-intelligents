def explain_integrated_gradients(model, tfidf, le, text, true_label, device, top_k=15, steps=50):
    """
    ...
    baseline = np.zeros_like(X)  # baseline = vecteur nul
    ...
    for target_class_idx, target_class_name in enumerate(le.classes_):
        ...
        # Logit sur l'input et sur la baseline
        with torch.no_grad():
        ...
        print(f"\n INTÉGRATION DES GRADIENTS (baseline → input)")

        integrated_grads = np.zeros_like(X[0])

        # Intégration le long du chemin
        for step in range(steps):
            ...
            # Gradient par rapport à l'entrée interpolée
        ...
        # Moyenne et multiplication par (input - baseline)
        ...
        print(f"   {'Rang':<6} {'Terme':<30} {'Attribution':<15} {'TF-IDF':<10}")
        print(f"   {'-'*65}")
        for rank, (term, attr, tfidf_val) in enumerate(positive[:top_k], 1):
            print(f"   {rank:<6} {term:<30} {attr:+.6f}       {tfidf_val:.4f}")
